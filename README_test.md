...
### Containers
Containers are vanilla ore custom block. I will present the custom block system.

They are function to call every time you do something with a container.
...
- *``Slot`` : The slot number of the container, you can declare multiple time a slot.
...
- ``filters`` : The filter of the slot (can be custom or integrated), see below for more informations.
...
For example here the filter will allow ``minecraft:stone`` or ``minecraft:dirt`` with ``energy`` tag set to 1b OR ``minecraft:coal``.

The filter can be customised (see function calls)


### Cables

Cables are custom block that handle a cable system between all device of the same network.

They are function to call every time you do something with a cable.
...
```
    63 -> 1  1  1  1  1  1 -> all sides are active
	      32 16 8  4  2  1
          E  W  S  N  U  D
    This can be use to update the model of the cable.
```

### Servos

Servos are custom block part of a network that can input/output items from/to containers.

They are function to call every time you do something with a servo.
...
__Outputs__:
  - ``storage itemio:io output`` : The item that could not be inputed
  - ``score itemio.io #success_input`` : Set to 1 if the input was successful, 0 otherwise
  - ``score itemio.io #count_to_remove`` : The number of item that could not be inputed
...
__Inputs__:
  ...
  - ``storage itemio:io output_side`` : The side to output to can be :
      ``"north","south","east","west","top","bottom","wireless"`` (wireless bypass the side check)
  - ``storage itemio:io input`` : If declared, the item will do an NBT check before outputing
  - ``storage itemio:io filters`` : If declared, the item will do a filter check before outputing (see above)

__Outputs__:
  - ``storage itemio:io output`` : The item outputed from the container
...
```
Used to add custom filter, implement your own syntax here. (see internal function as a example)
