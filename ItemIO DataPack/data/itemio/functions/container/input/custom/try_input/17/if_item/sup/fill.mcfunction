execute store result storage itemio:main.input Items[{Slot:17b}].Count byte 1 run scoreboard players get #full_stack itemio.math.input
execute store result block ~ ~ ~ Items[{Slot:17b}].Count byte 1 run scoreboard players get #full_stack itemio.math.input
