execute store result score #count_container itemio.math run data get storage itemio:main.input Items[{Slot:14b}].Count
...
execute if score #new_count_container itemio.math <= #full_stack itemio.math run function itemio:container/input/custom/try_input/14/if_item/inf/test_nbt
execute if score #new_count_container itemio.math > #full_stack itemio.math if score #count_container itemio.math < #full_stack itemio.math run function itemio:container/input/custom/try_input/14/if_item/sup/test_nbt
