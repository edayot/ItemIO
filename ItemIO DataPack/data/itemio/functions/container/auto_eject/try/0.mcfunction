...
data modify storage itemio:io input set from storage itemio:main Items2[{Slot:0b}]
...
execute if data storage itemio:main ioconfig2[0].allowed_side{north:1b} positioned ~ ~ ~-1 run function itemio:container/auto_eject/try/side/south
...
execute if score #success_input itemio.math matches 1 run item modify block ~ ~ ~ container.0 itemio:input/remove_count_origin
