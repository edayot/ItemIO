...
data modify storage itemio:io input set from storage itemio:main Items2[{Slot:XXXb}]
...
execute if score #success_input itemio.math matches 1 run item modify block ~ ~ ~ container.XXX itemio:input/remove_count_origin

""".replace("XXX",str(i))
    with open(str(i)+".mcfunction","w") as f:
        f.write(main)
    print("execute if score #slot2 itemio.math matches XXX if data storage itemio:main Items2[{Slot:XXXb}] run function itemio:container/auto_eject/try/XXX".replace("XXX",str(i)))
